...
ofRectangle transformRect(ofRectangle rect,ofMatrix4x4 mat) {
    ...
    for (int i=0; i<4; i++) {
        corners.push_back(mat.preMult(getCorner(rect,i)));
    }
    
    ofPoint minp;
    ofPoint maxp;
    
    minp=maxp=corners.front();;
    
    
    for (vector<ofPoint>::iterator iter=corners.begin();iter!=corners.end();iter++) {
        minp.x=min(minp.x,iter->x);
        minp.y=min(minp.y,iter->y);
        maxp.x=max(maxp.x,iter->x);
        maxp.y=max(maxp.y,iter->y);
    }
